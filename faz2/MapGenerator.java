...
public class MapGenerator {
    ...
    String[] score=null;
    String[] bombX =new String[6];
    String [] bombY=new String[6];
    ...
    void setBombs() {
        ...
        for(int i=0;i<6;i++) {
            ...
            } while (!isgreen(x, y) || repetetiveBomb(x,y,i) || isplayer(x, y));
            bombX[i] = Integer.toString(x);;
            bombY[i] = Integer.toString(y);;
        }
    }
    ...
    boolean repetetiveBomb(int x,int y,int i){
        String k= String.valueOf(x);
        String n= String.valueOf(y);
        for(int j=0;j<i;j++){
            if(bombX[j].equals(k) || bombY[j].equals(n))
                return true;
        }
        return false;
    }
